...
def current_user(authorization: str = Header(default=""), db: Session = Depends(db_session)) -> User:
    ...
    try:
        raw = base64.urlsafe_b64decode(authorization[7:].encode()).decode().split(":")
        user_id, issued, sig = int(raw[0]), raw[1], raw[2]
        payload = f"{user_id}:{issued}"
        expected = hmac.new(SECRET.encode(), payload.encode(), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(sig, expected) or time.time() - int(issued) > 86400 * 7:
            raise ValueError
    except Exception:
